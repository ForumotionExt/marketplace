(function () {
  ...
  const SEOPanel = {
    nav: () => {
      ...
      navItem.innerHTML = `<a href="${origin}/admin/?part=fm&sub=plugins&mode=seo&extended_admin=1&tid=${tid}"><span>SEO Tools</span></a>`;
      return navItem;
    },

    init: (container) => {
      // Afișăm panoul doar pe pagina corectă
      if (!window.location.href.includes('part=fm&sub=plugins&mode=seo')) return;
